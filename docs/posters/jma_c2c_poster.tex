...
\begin{column}{0.295\textwidth}
  ...
  \begin{block}{Results and Future Work}
    ...
    association because the sample is small, selected, and used to tune the
    detector. Next steps to take as soon as we get more annotated scans include: 

    \vspace{0.3cm}
    \begin{enumerate}
      \item \textbf{Test correlations with functional measurements.} Evaluate
        whether barcoding and EA presence, interval count, width, location,
        texture, and depth persistence are associated with available functional
        measurements and subsequent progression outcomes.
      \item \textbf{Develop a hidden Markov model (HMM).} Use spatial context
        and transitions between neighboring scan columns to improve automated
        EA and barcoding detection while reducing isolated false-positive
        regions.
    \end{enumerate}
  \end{block}
  ...
  \begin{block}{Acknowledgments}
    \vspace{0.3cm}
    I would like to thank Yingjia Wei and Monika Fleckenstein (University of Utah,
    Department of Ophthalmology and Visual Sciences) for their contributions.
    Supported by NIH award
    \href{https://reporter.nih.gov/search/IkRjh0_6kUCR974YUaREQQ/project-details/10635325}%
    {\textbf{1R01EY034965-01}}.
  \end{block}
