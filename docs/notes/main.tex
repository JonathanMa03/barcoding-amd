...
* Run `git clone https://github.com/Translational-Biophotonics-Laboratory/octvision3d.git`  into root/external

* Replace the git part of the gitmodule with https so it looks like:

\begin{lstlisting}
[submodule "nnUNet"]
    path = nnUNet
    url = https://github.com/dkermany/nnUNetOCT.git
\end{lstlisting}

* Run `git submodule sync` and`git submodule update --init --recursive` to pull the submodule''
\end{document}
